...
//            -------> Vec in Ray with only two components ?    <----------
//         var ray = new Ray(new Point(1.0f, 1.0f, 0.0f), new Vec(-1.0f, -1.0f)); 

//         var intersection = sphere.ray_intersection(ray);
// // We normalize "intersection.normal", as we are not interested in its length
//         Assert.True((intersection.Normal.normalize()).is_close(new Normal(1.0f, 4.0f, 0.0f).normalize()));
//     }
    ...
    public void TestNormalDirection()
    {
        ...
        var intersection = sphere.ray_intersection(ray); 
        // We normalize "intersection.normal", as we are not interested in its length
        Assert.True(intersection.Normal.normalize().is_close(new Normal(0.0f, 1.0f, 0.0f).normalize()));
    }
