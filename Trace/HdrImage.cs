...
public class HdrImage
{
    ...
    public HdrImage(int width, int height)
    {
        ...
        for (var i = 0; i < Height * Width; i++) Pixels[i] = new Color();
    }
    ...
    public string read_line(Stream inputStream)
    {
        ...
        while (true)
        {
            var curByte = binReader.ReadBytes(1);
            var curString = Encoding.ASCII.GetString(curByte);

            if (curString == "\n" || curString == "") return result;
    ...
    public float read_float(Stream inputStream, bool endiannessBool)
    {
        ...
        if (!endiannessBool) Array.Reverse(curBytes);
    ...
    public bool parse_endianness(string line)
    {
        ...
        if ((endianness < 0 && !BitConverter.IsLittleEndian) ||
            (endianness > 0 && BitConverter.IsLittleEndian)) return false;

        if (endianness == 0) throw new InvalidPfmFileFormat("Invalid endianness specification, it cannot be zero.");
    ...
    public (int, int) parse_img_size(string line)
    {
        ...
        if (elements.Length != 2) throw new InvalidPfmFileFormat("Invalid image size specification.");
        ...
        try
        {
            ...
            if (width <= 0 || height <= 0) throw new InvalidPfmFileFormat("width and height must be positive.");
        }
        catch (FormatException err)
        {
    ...
    {
        double cumSum = 0;
        for (var i = 0; i < Pixels.Length; i++) cumSum += Math.Log10(Pixels[i].luminosity() + delta);
